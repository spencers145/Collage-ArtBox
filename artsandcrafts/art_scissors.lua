    ...
    use = function(self, card, area)
	  ...
	  if selected.edition and #G.consumeables.cards + G.GAME.consumeable_buffer < G.consumeables.config.card_limit then
		...
		G.E_MANAGER:add_event(Event({
                    func = function()
                        ArtBox.create_edition_collectable(selected.edition.key)
	  ...
	  if selected.seal and #G.consumeables.cards + G.GAME.consumeable_buffer < G.consumeables.config.card_limit then
		...
		G.E_MANAGER:add_event(Event({
                    func = function()
                        ArtBox.create_seal_collectable(selected.seal)
                        G.GAME.consumeable_buffer = 0
                        return true
                    end
        }))
	  ...
	  if selected.config.center ~= G.P_CENTERS.c_base and #G.consumeables.cards + G.GAME.consumeable_buffer < G.consumeables.config.card_limit then
		...
		G.E_MANAGER:add_event(Event({
                    func = function()
			            ArtBox.create_enhancement_collectable(selected.config.center.key)
                        G.GAME.consumeable_buffer = 0
                        return true
                    end
        }))
